  ...
  return (
    <div className="modal-overlay active" style={{ display: 'flex' }} onClick={onCancel}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '400px' }}>
        <p style={{ margin: '0 0 1.5rem', fontSize: '1rem', color: 'var(--text-primary)' }}>
          {message || t('common.confirm_delete') || 'Are you sure?'}
        </p>
        <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
          <button
            onClick={onCancel}
            style={{ padding: '0.5rem 1rem', borderRadius: '8px', background: 'var(--bg-secondary)', border: '1px solid var(--glass-border)', color: 'var(--text-primary)', cursor: 'pointer' }}
          >
