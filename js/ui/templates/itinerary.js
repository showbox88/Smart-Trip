...
export function getTimelineItemHTML(day, stop, index, locationIdx, showTransit, showAddRow) {
    let contentHtml = '';
    let stayLineHtml = '';
    let transitStayLineHtml = '';
    const stayId = day.currentDayStay?.id;
    ...
    if (day.currentDayStay) {
        ...
        const isCoutItem = stop.id === day.currentDayStay.checkoutStopId;

        // Vertical range for the card segment
        let lineTop = '0';
        let lineBottom = '0';
        let showLine = false;
        ...
        if (day.isBetween) {
            ...
            lineBottom = '-0.8rem';
            showLine = true;
        } else if (day.isCinOnly || day.isSameDayStay) {
            if (isCinItem) {
                lineTop = '50%';
                lineBottom = '-0.8rem';
                showLine = true;
            } else if (day.isAfterCin(index)) {
                lineTop = '-0.8rem';
                lineBottom = '-0.8rem';
                showLine = true;
            }
        } else if (day.isCoutOnly) {
            if (isCoutItem) {
                lineTop = '-0.8rem';
                lineBottom = '50%';
                showLine = true;
            } else if (day.isBeforeCout(index)) {
                lineTop = '-0.8rem';
                lineBottom = '-0.8rem';
                showLine = true;
            }
        ...
        if (day.isSameDayStay) {
            if (isCinItem) {
                ...
                lineBottom = '50%';
                showLine = true;
            } else if (day.isAfterCin(index) && day.isBeforeCout(index)) {
                lineTop = '-0.8rem';
                lineBottom = '-0.8rem';
                showLine = true;
            }
        ...
        if (showLine) {
            stayLineHtml = `<div class="stay-line-segment" data-stay-id="${stayId}" style="position:absolute; left:10px; top:${lineTop}; bottom:${lineBottom}; width:4px; background:${stayColor}; opacity:1; z-index:1; border-radius: 2px; transition: all 0.3s ease;"></div>`;
        }

        // Vertical segment for the transit row below
        let showInTransit = false;
        if (day.isBetween) showInTransit = true;
        else if (day.isCinOnly || day.isSameDayStay) {
            if (index >= cinIdx && (coutIdx === -1 || index < coutIdx)) showInTransit = true;
        } else if (day.isCoutOnly) {
            if (index < coutIdx) showInTransit = true;
        }

        if (showInTransit) {
            transitStayLineHtml = `<div class="stay-line-segment" data-stay-id="${stayId}" style="position:absolute; left:10px; top: -0.8rem; bottom: -0.8rem; width:4px; background:${stayColor}; opacity:1; z-index:1; border-radius: 2px; transition: all 0.3s ease;"></div>`;
        }
    }
    ...
    const styleBlock = index === 0 ? `
        <style>
            ...
            /* Shared Stay Hover Styles */
            .stay-line-segment.stay-hover {
                opacity: 1 !important;
                background-color: #ffb400 !important;
                box-shadow: 0 0 15px rgba(245, 158, 11, 0.8) !important;
                width: 5px !important;
            }
            .rich-stop-card.comfy-node-link.stay-hover {
                --hotel-accent: #ffb400 !important;
                --hotel-glow: 0 0 15px rgba(245, 158, 11, 0.5) !important;
                box-shadow: var(--hotel-glow), 0 5px 15px rgba(0, 0, 0, 0.4) !important;
            }
            .rich-stop-card.stay-hover::after {
                height: 4px !important;
                opacity: 1 !important;
                background-color: #ffb400 !important;
                box-shadow: 0 0 12px rgba(245, 158, 11, 0.6) !important;
            }
            .rich-stop-card.stay-hover::before {
                transform: translate(-50%, -50%) scale(1.3) !important;
                border-color: #ffb400 !important;
                box-shadow: 0 0 15px rgba(245, 158, 11, 0.8), 0 0 6px rgba(255, 255, 255, 1) !important;
            }
        </style>
    ` : '';
...
export function getDayHTML(day, dayIndex, activeDayId) {
    ...
    if (trip) {
        ...
        allStops.forEach(s => {
            if (s.stayId) {
                if (!staysMap.has(s.stayId)) staysMap.set(s.stayId, { id: s.stayId, checkinDayId: null, checkoutDayId: null, checkinStopId: null, checkoutStopId: null, location: s.location });
                const stay = staysMap.get(s.stayId);
                if (s.type === 'hotel_checkin') {
                    stay.checkinDayId = s.dayId;
                    stay.checkinStopId = s.id;
                }
                if (s.type === 'hotel_checkout') {
                    stay.checkoutDayId = s.dayId;
                    stay.checkoutStopId = s.id;
                }
            }
        });
    ...
    // Enrich day object for stay logic propagation
    day.currentDayStay = currentDayStay;
    day.isCinOnly = isCinOnly;
    day.isCoutOnly = isCoutOnly;
    day.isBetween = isBetween;
    day.isSameDayStay = isSameDayStay;

    const cinIdx = currentDayStay ? day.stops.findIndex(s => s.id === currentDayStay.checkinStopId) : -1;
    const coutIdx = currentDayStay ? day.stops.findIndex(s => s.id === currentDayStay.checkoutStopId) : -1;
    ...
    return `
        <div class="day-section" id="${day.id}" style="margin-bottom: 3rem; scroll-margin-top: 120px;"
            ...
            if (day.stops) day.stops.forEach(stop => parseTransit(stop.transitToNext));
            if (day.showReturnRoute) parseTransit(day.transitToHotel);

            if (!hasData) return t('itinerary.no_data');
            return `${formatDuration(totalSeconds)}, ${formatDistance(totalMeters)}`;
        })()}</span>
                ...
                <div class="timeline-container" style="position: relative;">
                    ...
                    ${(isBetween || isCoutOnly) ? `
                        <div class="hotel-hint-row" ${hoverSyncAttrs} style="padding-left: 36px; margin-bottom: 0.8rem; color: #f59e0b; font-size: 0.85rem; font-weight: 600; display: flex; flex-direction:column; gap: 4px; position: relative; cursor: pointer;">
                            ...
                            <div style="font-weight: normal; color: var(--text-secondary); font-size: 0.8rem; display:flex; align-items:center; gap: 0.6rem;">
...
export function getTripHTML(trip) {
    if (!trip) return `<h2>${t('common.not_found')}</h2>`;
    ...
    return `
        ...
        return `
                            <li id="nav-day-${day.id}" class="${day.id === trip.activeDayId ? 'active' : ''}" onclick="scrollToDay('${day.id}')" style="--active-color: ${activeColor};">
                                <div class="nav-day-main" style="display:flex; align-items:center; gap: 10px; min-width:0; width: 100%;">
                                    ...
                                    <span class="nav-day-short">D${index + 1}</span>
                                    <span class="nav-day-title" style="white-space:nowrap; font-size:0.9rem; color:var(--text-primary); font-weight:600;">${dayLabel}</span>
                                    <span class="nav-day-date" style="font-size:0.75rem; color:var(--text-secondary); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; margin-left: auto;">${formatDate(day.date).split(',')[0]}</span>
                                </div>
                        ...
                        <div style="display:flex; align-items:center; gap: 1rem;">
                            ...
                            <div>
                                ...
                                <div style="display:flex; align-items:center; gap:0.6rem; margin-top:0.3rem; flex-wrap:wrap;">
                                    ...
                                    <span class="header-badge">${calculateDays(trip.startDate, trip.endDate)} ${t('itinerary.days')}</span>
                                    ${totalCost > 0 ? `<span class="header-badge">${formatCurrency(totalCost)}</span>` : ''}
                                </div>
                            </div>
                        </div>
                        <div style="position:relative;">
                            <button class="menu-dots" onclick="toggleMenu(event, 'header-${trip.id}')">⋮</button>
